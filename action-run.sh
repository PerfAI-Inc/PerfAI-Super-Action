...
TEMP=$(getopt -n "$0" -a -l "hostname:,username:,password:,openapi-url:,base-path:,api-id:,label:,wait-for-completion:,fail-on-new-leaks:,auth_url_1:,auth_body_1:,auth_headers:,auth_url_2:,auth_body_2:,auth_headhers:" -- -- "$@")
...
do
    case "$1" in
        ...
        --openapi-url) OPENAPI_URL="$2"; shift;;
        --base-path) BASE_PATH="$2"; shift;;        
        --api-id) CATALOG_ID="$2"; shift;;
        --label) LABEL="$2"; shift;;
